...
public abstract class ProductComparisonJakarta {
    public static final int TX_TIMEOUT = 50;
    protected static final String outerClassName = ProductComparisonJakarta.class.getName();
    ...
    private ProductWorkerJakarta<Void> worker;

    public static void main(String[] args) throws RunnerException, CommandLineOptionException, CoreEnvironmentBeanException {
        JMHConfigJTA.runJTABenchmark(ProductComparisonJakarta.class.getSimpleName(), args);
    }

    protected abstract ProductInterfaceJakarta getProductInterface();

    protected ProductWorkerJakarta<Void> getProductWorker() {
        return new ProductWorkerJakarta<Void>(getProductInterface());
    }
    ...
    protected void doWork(ProductInterfaceJakarta product) throws Exception {
        try {
            worker.doWork();
        } catch (Exception e) {
